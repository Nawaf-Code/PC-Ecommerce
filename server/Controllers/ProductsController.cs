...
using Newtonsoft.Json; // Ensure this namespace is added
using server.Data;
using server.DTOs;
using server.Models;
...
public class ProductsController : BaseController
{
    ...
    private readonly ILogger<ProductsController> _logger; // Define a logger

    public ProductsController(UserManager<User> userManager, CommerceContext context, ILogger<ProductsController> logger)
    {
...
)
{
    ...
    var categoryExists = _context.Categories.Any(c => c.Id == productCreateDTO.CategoryId);
    if (!categoryExists)
    {
        return BadRequest($"Category with ID {productCreateDTO.CategoryId} does not exist.");
    }

    var subcategoryExists = _context.Subcategories.Any(sc => sc.Id == productCreateDTO.SubcategoryId);
    if (!subcategoryExists)
    {
        return BadRequest($"Subcategory with ID {productCreateDTO.SubcategoryId} does not exist.");
    }
    ...
    {
        _logger.LogError($"An error occurred while saving the entity changes: {ex.InnerException?.Message}");
